...
func TestCommunicator_GlobalWritesAttributes(t *testing.T) {
	t.Run("requests a write of attributes from provider and returns responses", func(t *testing.T) {
		mockProvider := &zigbee.MockProvider{}
		cr := zcl.NewCommandRegistry()
		global.Register(cr)
		...
		var expectedRequestOne = zcl.Message{
			FrameType:           zcl.FrameGlobal,
			Direction:           zcl.ClientToServer,
			TransactionSequence: transactionSequence,
			Manufacturer:        zigbee.NoManufacturer,
			...
			Command: &global.WriteAttributes{
				Records: []global.WriteAttributesRecord{
					{
						Identifier: 0x0004,
						DataTypeValue: &zcl.AttributeDataTypeValue{
		...
		expectedResponse := &global.WriteAttributesResponse{
			Records: []global.WriteAttributesResponseRecord{
				{
					Identifier: 0x0004,
					Status:     0,
				},
		...
		appRequestOne, err := cr.Marshal(expectedRequestOne)
		assert.NoError(t, err)
